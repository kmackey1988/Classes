...
function createPost($request_values)
	{
		global $conn, $errors, $title, $featured_image, $genre_id, $body, $published;
		$title = esc($request_values['title']);
		$body = htmlentities(esc($request_values['body']));
		if (isset($request_values['genre_id'])) {
		...
		if (empty($genre_id)) { array_push($errors, "Post genre is required"); }
		// Get image name
		...
		if (count($errors) == 0) {
			...
			if(mysqli_query($conn, $query)){ // if post created successfully
				...
				// create relationship between post and genre
				$sql = "INSERT INTO post_genre (post_id, genre_id) VALUES($inserted_post_id, $genre_id)";
	...
	* * * * * * * * * * * * * * * * * * * * * */
	...
	{
		global $conn, $errors, $post_id, $title, $featured_image, $genre_id, $body, $published;
		...
		if (count($errors) == 0) {
			...
			// attach genre to post on post_genre table
			if(mysqli_query($conn, $query)){ // if post created successfully
				if (isset($genre_id)) {
					$inserted_post_id = mysqli_insert_id($conn);
					// create relationship between post and genre
